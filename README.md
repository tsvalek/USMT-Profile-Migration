...
│   │   └── MigCustom.xml          ← Доп. миграция (диски, принтеры, закладки)
...
- **Подключённые сетевые диски** (буквы и пути, без паролей)
- **Подключённые сетевые принтеры** (включая принтер по умолчанию)
- **Закладки браузеров** (Chrome, Edge, Firefox, Opera, Brave, IE)
- **История браузеров** (где поддерживается)
- Настройки браузеров (расширения, предпочтения)
...
- **Пароли сетевых дисков** — не переносятся (нужно ввести заново)
- **Пароли браузеров** — не переносятся (ограничение Windows Credential Manager)
- **Локальные принтеры** — не переносятся (только сетевые)
